...
//                               Ethernet, Magic Mouse 2, Magic Keyboard -
//                             </textarea>
//                           </div>
//                         </div>
...
export default function UpdateProduct({
  ...
  const { _id, name, manufacturer } = updateProductData;
...
const validateFields = () => {
  ...
  const updateProduct = () => {
    ...
    fetch(`${serverUrl}/product/update`, {
      ...
      .then((result) => {
        if (!result.ok) throw new Error("Failed to update product");
        ...
        updateModalSetting(); // Notify parent to refresh
      })
      .catch((err) => {
        console.error("Error updating product:", err);
        Swal.fire({
