...
export default function AddStore({ onStoreAdded }) {
  ...
  const validateForm = () => {
    const { name, category, address, city, image } = form;

    // Empty check
    if (!name || !category || !address || !city || !image) {
      setError("All fields are required.");
      return false;
    }

    // ✅ Length check
    if (name.length > 25 || city.length > 25) {
      setError("Name and City must not exceed 25 characters.");
      return false;
    }

    // ✅ Allowed only letters, numbers, and spaces
    const allowed = /^[A-Za-z0-9 ]+$/;
    if (!allowed.test(name) || !allowed.test(city)) {
      setError("Name and City can only contain letters, numbers, and spaces.");
      return false;
    }
    ...
    await fetch("https://api.cloudinary.com/v1_1/dylttfesj/image/upload", {
      ...
      .then((data) => {
        ...
        Swal.fire({
          ...
          text: "Store image uploaded successfully.",
        });
      })
      .catch((error) => console.error("Error uploading image:", error));
  };
                          ...
                          <div>
                            ...
                            <input
                              ...
                              onKeyDown={(e) => {
                                ...
                                if (invalid.test(e.key)) e.preventDefault(); 
                              }}
                              maxLength={25}
                              className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5"
                              placeholder="Enter store name"
