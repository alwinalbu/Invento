...
function Suppliers() {
  ...
  const fetchSuppliers = async () => {
    try {
      const res = await fetch(`${serverUrl}/supplier/get/${authContext.user}`);
  ...
  //   const supplierData = {
  //     ...newSupplier,
  //     userId: authContext.user, // ✅ attach logged-in user ID
  //   };
  ...
  const handleAddSupplier = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.errors) {
          return Swal.fire("Validation Error", data.errors.join("\n"), "error");
        }
  ...
  const handleEditSupplier = async (e) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return Swal.fire("Error", data.message || "Update failed", "error");
      }
